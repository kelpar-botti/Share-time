...
3. 「この日の予約を申請する」から、開始時刻・終了時刻・予定名・お名前・内容を入力して申請（例: 13:00〜15:00）
   - **24時間いつでも申請可能**。終了時刻に開始時刻より早い時間を選ぶと「日をまたぐ予約」（例: 22:00〜翌2:00）として扱われます
   - お名前・内容は必須、メールアドレスは任意（結果をメールで受け取りたい場合のみ記載）
   - 入力内容は他の利用者には一切表示されません。管理者だけが確認できます（フォーム上にもその旨を明記）
   - **予定名だけは例外的に公開できます。** 「この予定名を他の利用者にも公開する」にチェックを入れた場合のみ、他の訪問者にも時間と一緒に予定名（例:
     「歯医者」）が表示されます。チェックしなかった場合、その予定は非公開のままで、**管理者が後から勝手に公開へ変更することはできません**（許可した場合のみ、管理者が公開・非公開を切り替え可能）
   - この設定は申請者本人だけが後からいつでも変更できます。申請完了画面（および確定・見送りメール）に、本人専用の設定変更リンク（`/title/[id]?token=...`）が表示されます
4. 送信すると、その時間帯は即座に×（申請中）になり、あなたのGmailに通知メールが届く（予定名・お名前・内容も記載されます）
5. 通知メール内の「承認する」「却下する」リンクをクリックするだけで確定（管理画面へのログイン不要）
   - もちろん `/admin` の管理画面からまとめて承認/却下することも可能。予約の内容は承認待ち・履歴どちらの一覧でも確認できます
   - 承認・却下のどちらでも、任意で返信メッセージを添えられます（相手がメールアドレスを記載していた場合のみ送信）
6. 申請者がメールアドレスを記載していた場合のみ、承認・却下の結果を自動通知（未記載の場合は送られません）
7. **承認された予約は、申請者本人が時間を延長・短縮できます。** `/title/[id]` の管理ページから新しい開始/終了時刻を選んで申請すると、
   予約は一旦「承認待ち」に戻り、管理者に再承認を依頼するメールが届きます（承認・却下どちらの結果も、通常の申請と同じくメールで通知されます）

### 管理者（あなた）側

- `/admin` … 承認待ちの申請一覧（承認/却下）と、これまでの履歴
  - 各申請には、その日付を含む月の混雑度カレンダー（公開ページと同じ表示）が一緒に表示され、承認/却下を判断する前に前後の予定状況を確認できます
- `/admin/schedule` … バイトなど、あなた自身の予定を直接ブロックする画面
  - **予定名の公開・非公開を選択可能**。初期状態は非公開で、公開にチェックした場合のみ、日別の予約状況・予約フォームに予定名が表示されます
  - 登録済み一覧の「予定名を公開する／非公開にする」で後から変更できます。以前の予定（旧メモ）も初期状態では非公開のままです
  - 月を移動しても予定名・時間・選択日を保持し、複数月の日付をまとめて選べます。送信中はボタンが無効になります
...
src/
  app/
    ...
    title/[id]/page.tsx                申請者本人が予定名の公開設定・予約時間を変更するページ（トークン必須）
    r/[id]/page.tsx                    メール内の承認/却下リンクの着地ページ
    admin/login/page.tsx               管理者ログイン画面
    admin/(protected)/layout.tsx       管理画面の認証ガード
    admin/(protected)/page.tsx         管理画面本体（承認待ち一覧・履歴・周辺カレンダー）
  ...
  components/
    MonthDensityCalendar.tsx           月間の混雑度カレンダー（公開ページ・管理画面で共用）
    AdminLoginLink.tsx                 公開ページ下部の管理者ログインリンク
  lib/
    date.ts                           日本時間の日付計算、営業時間、カレンダー生成
    firebase-admin.ts                  Firebase Admin SDK 初期化
    bookings.ts                       Firestoreへの予約データの読み書き（重複チェック・予定名の公開制御を含む）
    holidays.ts                       日本の祝日判定（@holiday-jp/holiday_jp のラッパー）
...
- `npm test`: 実データに接続せず、管理者の予定名公開・非公開、旧データ互換、訪問者の公開許可保護、未ログイン時の拒否、一括登録と日またぎ表示を検証します
- `npm run lint`: ソース・設定・テストの静的チェック（生成済みの `.next` は除外）
- `npm run build`: 本番用ビルドと型チェック
