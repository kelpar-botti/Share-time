...
| 日本の祝日判定 | [@holiday-jp/holiday_jp](https://github.com/holiday-jp/holiday_jp-js) | 2050年まで計算済みのデータをアプリ内に持つので、外部API通信なしで永久無料・オフラインで動作 |
...
1. トップページに月間カレンダーが表示され、予約が埋まっている日ほど濃い青で表示される（Googleマップの混雑度のようなイメージ）。日曜日と日本の祝日は数字が赤色で表示される。月の移動ボタンでかなり先の月まで確認可能
